...
if __name__ == '__main__':
	...
	df = pd.read_csv(fileTmpUtf8, skiprows=1, usecols=[0] + list(range(colIdx+1, colIdx+5)), parse_dates=[0])
	#  @TODO:  Define datetime range. Use last 1000 values for now.
	df = df[-100:]

	dates = df["日付"]
	df["日付"] = dates.values.astype("datetime64[D]").astype(float)

	step = df.shape[0] / 10
	plt.xticks(df["日付"][::step], [x.strftime("%Y/%m/%d") for x in dates[::step]], rotation=20)
